...
import { IMenuProp } from "@/type/type";
import route from "@/route";
const cx = classNames.bind(styles);
interface SideBarProps {}

const Students: IMenuProp = {
  ...
  data: [
    ...
    {
      id: 3,
      label: "Thêm môn học cho sinh viên",
      to: route.addSubjectforStudent,
    },
  ],
};
const MonHocs: IMenuProp = {
  title: "Quản lý Môn học",
  data: [
    {
      id: 11,
      label: "Danh sách Môn học",
      to: route.subjectList,
    },
    {
      id: 12,
      label: "Thêm môn học",
...
const Lop: IMenuProp = {
  ...
  data: [
    ...
    },
    {
      id: 23,
      label: "Xóa Lớp",
      to: route.deleteClass,
    },
  ],
};
const Majors: IMenuProp = {
  title: "Quản lý Ngành",
  data: [
    {
      id: 31,
  ...
  return (
    <div className={cx("wrap")}>
      <div className={cx("top")}>
        ...
        <div>
          <h3 className={cx("top-title")}>Admin Portal</h3>
        </div>
      </div>
      <div className={cx("main")}>
        <div className={cx("section")}>
          <Menu title={Students.title} MenuItems={Students.data}></Menu>
          <Menu title={MonHocs.title} MenuItems={MonHocs.data}></Menu>
          <Menu title={Lop.title} MenuItems={Lop.data}></Menu>
          <Menu title={Majors.title} MenuItems={Majors.data}></Menu>
        </div>
      </div>
    </div>
  );
